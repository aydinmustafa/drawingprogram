...
def initalize(cols, rows, showGrid=False):
    ...
    pygame.display.set_caption('MustafaCanAydinDrawingProgram')
...
label = Label(window, text='Enter number of columns and rows  ex:25,50) :  ')
                ...
                if pos[1] >= grid.height:  # If the mouse is below the main drawing grid
                    if pos[0] >= tools.startx and pos[0] <= tools.startx + tools.width and pos[1] >= tools.starty and \
                        ...
                        if clicked.text == 'D':  # Draw tool
                            ...
                            doFill = True
                        elif clicked.text == '-':  # Replace tool
                            replace = True
